...
{
    class CChannel;
    struct ManifestTableEntry_t;
    class CGenCPManifestSupport;
}
...
namespace uxapi
{
    ...
    // ----------------------------------------------------------------------
    // struct UxManifestDataItem
    /**
    \brief Definition of Xml manifest data item
    */
#pragma pack( push, 1)

    struct UxManifestTableEntry
    {
        uint32_t DataSize;
        enum Type
        {
        ...
        uint64_t Filesize;
        uint8_t  FileSha1Hash[20];
        uint32_t XmlSubminorVersion;
        uint32_t XmlMinorVersion;
        uint32_t XmlMajorVersion;
        uint32_t SchemaMajorVersion;
        uint32_t SchemaMinorVersion;
    };
#pragma pack(pop)
    ...
    */
    ...
    public:
        ...
        */
        ...
        /**
        \brief Return number of entries in the manifest table
        \param[out] pCount On success contains the number of entries in the manifest table.
        \return The status of the operation.
        */
        UXSTATUS GetManifestTableEntryCount( size_t* pCount );

        ///
        /**
        \brief Return a manifest table entry
        \param[in] index The index of the entry to return.
        \param[in][out] pEntry On success the buffer is filled with manifest data. You must set the UxManifestTableEntry.DataSize member before calling this function.
        \return The status of the operation.
        */
        UXSTATUS GetManifestTableEntry( size_t index, UxManifestTableEntry* pEntry );
...
#endif
        /**
        \brief Gets the device info.
        \param info The device info retrieved by device discovery.
        \return UX_OK if info is valid
        */
        ...
        enum EPropertyValue
        {
            ...
            */                                                                  //$
            ...
            /**                                                                 //$
                ...
                Last error status of a read or write operation.                 //$
            */                                                                  //$
            ...
            */                                                                  //$
            ...
            /// Security: Value of m_isBadMode in CChannel. The type is bool and only settable to true.
            BadModeValue = 0xFBAC7AC1,
            /// Security: Value of m_NextRequestID in CChannel. The type is uint16_t and only readable.
            RequestIdValue = 0xFB1267AC,
            /// Security: Value of m_SBRMOffset in CChannel. The type is uint64_t and only readable.
            SBRMOffset = 0xFB384DED
        };
...
/// SetProperty and GetProperty can be used to issue custom requests. Internal use only.
    ...
    private:
        ...
        GenCP_1_0::CGenCPManifestSupport* m_pManifest;
