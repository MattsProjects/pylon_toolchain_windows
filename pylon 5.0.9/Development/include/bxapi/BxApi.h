...
\param[out] pBuffer        The buffer for the returned string. The string is zero terminated and encoded in utf8 format. This parameter can be NULL to retrieve the required buffer size. If pBuffer is NULL BufferSize must also be 0.
...
\param[out] pRequiredSize  Returns the required size of the buffer in bytes including terminating 0.
If the buffer is not large enough to hold the full string, the returned string is truncated to bufferSize including terminating 0.
\return Status of the operation.
\threading
This function is thread-safe.
*/
