...
{
      BxapiStreamState_Invalid = 0   ///< Stream not found or device has been disconnected or the stream has been destroyed
...
You can use this handle on subsequent calls to BxapiStreamQueueBuffer() and BxapiStreamDeregisterBuffer().
...
*/
typedef void (BX_CALL *BxapiStreamBufferReadyCallback)(BxapiStreamHandle hStream, size_t imageReadyCount, void *userContext);
...
This function is thread-safe.
*/
BX_API BXSTATUS BxapiStreamRegisterCallback(BxapiStreamHandle hStream, BxapiStreamBufferReadyCallback callback, void *userContext);
...
/** \brief Value of ClockFrequency; The type is uint32_t; The clock frequency configured on the hardware grabber. (read and write) */                 //$
#define BXAPI_PROPERTYID_CLOCKFREQUENCY_VALUE 0x51000                                                                                                 //$
...
/** \brief Value of MissedFrameCount; The type is int64_t; The count of missed frames between grabbed images. (read only) */                          //$
#define BXAPI_PROPERTYID_MISSEDFRAMECOUNT_VALUE 0x50800                                                                                               //$
...
/** \brief Value of LastBlockId; The type is int64_t; The last grabbed block ID. (read only) */                                                       //$
#define BXAPI_PROPERTYID_LASTBLOCKID_VALUE 0x50A00                                                                                                    //$
                                                                                                                                                      //$
